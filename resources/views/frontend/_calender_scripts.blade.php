...
//                        alert("Start time: " + startHour + ":" + startMin + " " + startMeridiem + ", End time: " + endHour + ":" + endMin + " " + endMeridiem);
                        ...
                        var endDateObj   = new Date(parseInt(startYear),parseInt(startMonth - 1)  ,parseInt(startDay),startHour,startMin + 40,0,0);
                        var type         = $("#type").val();
                        var doctor       = $("#doctor").val();

                        var start_date = moment(startDateObj.toDateString() + " " + startDateObj.getHours() +":"+ startDateObj.getMinutes()).format("YYYY-MM-DD HH:mm:ss");
                        var data = {description: what, start_date: start_date, type: type,doctor_id:doctor,
                        _token:"{{csrf_token()}}", backgroundColor: $("#colorBackground").val(),
                        ...
                        $.ajax({
                            ...
                            error: function(xhr, type, exception) {
                                ...
                                swal(
                                    'Oops!',
                                    'Already an appointment schedule on this date!',
                                    'error'
                                )
                            }
                        });
                ...
                'Delete': function() {
                    if(confirm("Are you sure you want to delete this agenda item?")){
                        if(clickAgendaItem != null){
                            ...
                            $.ajax({
                                type  : 'POST',
                                url:  '{{ str_replace('-1','',route('frontend.delete-appointment',-1))  }}'+id,
                                data  : data,
...
//                            "<b>Ends:</b> " + endDate + "<br><br>"
        ...
        $.ajax({
            type  : 'POST',
            url   : '{{route("frontend.get-appointments")}}',
            data  : data,
            success: function (res) {
                ...
                $.each(res , function (index, obj) {
                    ...
                    var startDateObj = new Date(parseInt(startDate.format('YYYY')),parseInt(startDate.format('MM')-1) ,parseInt(startDate.format('DD')),parseInt(startDate.format('HH')),parseInt(startDate.format('mm')));
                    var endDateObj   = new Date(parseInt(endDate.format('YYYY')),parseInt(endDate.format('MM')-1) ,parseInt(endDate.format('DD')),parseInt(endDate.format('HH')),parseInt(endDate.format('mm')) + 40);

                    var description = obj.description ? obj.description : "Description";
                    jfcalplugin.addAgendaItem(
                        "#mycal",
                        description,
                        startDateObj,
